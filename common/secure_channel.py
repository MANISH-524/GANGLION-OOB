...
def derive_agent_keys(master_secret: bytes, agent_id: str) -> Tuple[bytes, bytes]:
    ...
    """
    info = b"ganglion-oob|agent|" + agent_id.encode("utf-8")
    enc_key = _hkdf(master_secret, b"ganglion-enc", info, 32)
    mac_key = _hkdf(master_secret, b"ganglion-mac", info, 32)
    return enc_key, mac_key


def load_master_secret(env_var: str = "GANGLION_MASTER_KEY",
    ...
    # Production deployments MUST set GANGLION_MASTER_KEY.
    return hashlib.sha256(b"ganglion-oob-development-master-key-CHANGE-ME").digest()
