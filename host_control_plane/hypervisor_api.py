...
logger = logging.getLogger("ganglion.hypervisor")
...
class ProxmoxConfig:
    ...
    api_user:        str = "ganglion@pam"
    api_token_name:  str = "ganglion-token"
    api_token_value: str = "CHANGEME-TOKEN-UUID"
    node:            str = "pve"
    quarantine_vlan: int = 30
    sandbox_vlan:    int = 40
    golden_snapshot: str = "GoldenImage-Clean"
    verify_ssl:      bool = True   # fail-safe default; set false only for lab self-signed certs
...
class HypervisorConfig:
    ...
    forensics_dir:   str = "/opt/ganglion-oob/host_control_plane/forensics_archive"
...
class ProxmoxBackend:
    ...
    def dump_memory(self, vm_id: str) -> HypervisorResult:
        ...
        ok, task = self._post(f"/vzdump", {
            ...
            "mode":    "snapshot",
            "compress": "zstd",
            "notes-template": f"ganglion-forensic-{ts}",
        })
...
class HypervisorAPI:
    ...
    def full_incident_response(self, vm_id: str) -> List[HypervisorResult]:
        ...
        # NOTE: the inter-step waits below exist ONLY to let a *real* hypervisor
        # settle (a genuine VM needs a moment after an ACPI action). When a step
        # fails because there is no hypervisor on this host, there is nothing to
        # wait for, so we skip the delay instead of faking ~9s of "work".
        step1 = self.isolate_vm(vm_id)
        results.append(step1)
        logger.info("Step 1 - Isolate: %s", step1.message)
        if step1.success:
            time.sleep(2)   # let the real NIC/VLAN change settle
        ...
        if step3.success:
            time.sleep(5)   # let a real snapshot restore complete
...
def load_default_config(config_path: Optional[str] = None) -> HypervisorConfig:
    ...
    cfg = HypervisorConfig(
        ...
        vm_name_map   = {
            "prod-vm-01": "GanglionTarget01",
            "prod-vm-02": "GanglionTarget02",
        },
    )
    return cfg
