#!/usr/bin/env python3
"""
Ganglion-OOB :: Host Network Containment
=========================================
...
class NullBackend(ContainmentBackend):
    ...
    def _isolate_cmds(self, mgmt_allow):
        base = ["ganglion-null", "isolate", "--drop-all-except-established"]
        if mgmt_allow:
            base += ["--allow", mgmt_allow]
        return [base]

    def _lift_cmds(self):
        return [["ganglion-null", "lift", "--restore-default-policy"]]
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Ganglion-OOB host containment")
