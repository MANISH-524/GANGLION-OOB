#!/usr/bin/env bash
# =============================================================================
# Ganglion-OOB :: Deployment Script
# =============================================================================
# Installs and configures Ganglion-OOB on either the Host Control Plane
...
ROLE="${1:-}"
INSTALL_BASE="/opt/ganglion-oob"
SERVICE_USER="ganglion"
PYTHON="python3"
...
if [[ "$ROLE" != "host" && "$ROLE" != "guest" ]]; then
    echo -e "${CYAN}"
    echo "  GANGLION-OOB DEPLOYMENT SCRIPT"
    echo -e "${RESET}"
...
if [[ "$ROLE" == "host" ]]; then
    ...
    mkdir -p /etc/ganglion-oob
    if [[ ! -f /etc/ganglion-oob/master.env ]]; then
        MASTER_HEX="$($PYTHON -c 'import os;print(os.urandom(32).hex())')"
        echo "GANGLION_MASTER_KEY=${MASTER_HEX}" > /etc/ganglion-oob/master.env
        chmod 600 /etc/ganglion-oob/master.env
        ok "Generated master key: /etc/ganglion-oob/master.env (mode 600)"
        warn "Copy this SAME file to every guest VM at /etc/ganglion-oob/master.env"
        echo -e "  ${CYAN}${MASTER_HEX}${RESET}"
    else
        info "Master key already present at /etc/ganglion-oob/master.env"
    fi
    ...
    cp "$SCRIPT_DIR/host_control_plane/ganglion-control.service" /etc/systemd/system/
    # Patch install path if non-default
    sed -i "s|/opt/ganglion-oob|${INSTALL_BASE}|g" /etc/systemd/system/ganglion-control.service
    systemctl daemon-reload
    systemctl enable ganglion-control.service
    ok "Systemd service installed: ganglion-control.service"
    ...
    echo -e "    2. Start: ${CYAN}sudo systemctl start ganglion-control${RESET}"
    echo -e "    3. Dashboard: ${CYAN}http://$(hostname -I | awk '{print $1}'):5000${RESET}"
    echo -e "    4. Logs: ${CYAN}sudo journalctl -u ganglion-control -f${RESET}"
    echo ""
    ...
    mkdir -p /etc/ganglion-oob
    if [[ ! -f /etc/ganglion-oob/master.env ]]; then
        warn "No /etc/ganglion-oob/master.env found on this guest."
    ...
    info "Installing systemd service..."
    cp "$SCRIPT_DIR/guest_production_vm/ganglion-sentry.service" /etc/systemd/system/

    # Prompt for controller IP
    echo ""
    read -r -p "  Enter Control Center IP (VLAN 20, e.g. 192.168.20.1): " CTRL_IP
    CTRL_IP="${CTRL_IP:-192.168.20.1}"
    sed -i "s|192.168.20.1|${CTRL_IP}|g" /etc/systemd/system/ganglion-sentry.service
    sed -i "s|/opt/ganglion-oob|${INSTALL_BASE}|g" /etc/systemd/system/ganglion-sentry.service

    read -r -p "  Enter target directory to monitor (default: /home): " TARGET_DIR
    TARGET_DIR="${TARGET_DIR:-/home}"
    sed -i "s|--target-dir /home|--target-dir ${TARGET_DIR}|g" /etc/systemd/system/ganglion-sentry.service

    systemctl daemon-reload
    systemctl enable ganglion-sentry.service
    ok "Systemd service installed: ganglion-sentry.service"
    ...
    echo ""
    echo -e "  Next steps:"
    echo -e "    1. Start: ${CYAN}sudo systemctl start ganglion-sentry${RESET}"
    echo -e "    2. Verify (should be silent): ${CYAN}sudo systemctl status ganglion-sentry${RESET}"
