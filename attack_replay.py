#!/usr/bin/env python3
"""
Ganglion-OOB :: Attack Replay & Detection Validation
=====================================================
...
def run_offline() -> int:
    import logging
    logging.getLogger("ganglion.hypervisor").setLevel(logging.WARNING)
    ...
    print(f"   False-positive rate           : {p('cyn', f'{fp_rate:.0f}%')} "
          f"({fp}/{len(benign_cases)} benign control case(s))")
...
def main():
    ap = argparse.ArgumentParser(description="Ganglion attack-replay validation")
