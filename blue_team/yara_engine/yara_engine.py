#!/usr/bin/env python3
r"""
Ganglion-OOB :: Blue Team Tool 6 — YARA-like Rule Engine
=========================================================
...
Rule file syntax (.vyr = Ganglion YARA):
...
logger = logging.getLogger("ganglion.yara_engine")
...
class VYRParser:
    """Parser for .vyr rule files (Ganglion YARA Rule format)."""
...
class RuleMatcher:
    ...
    def _eval_condition(self, rule: YarRule, data: bytes,
        ...
        # ── Security gate ──────────────────────────────────────────────────
        # After every legitimate token has been substituted to True/False or a
        # number, the ONLY things that may remain are the boolean keywords,
        # numbers, comparison/arithmetic operators, parentheses and whitespace.
        # Anything else (a stray identifier such as __class__, an attribute
        # access, a call) means the condition contained something we do not
        # support — which, for rules pulled from untrusted feeds, is exactly the
        # eval() sandbox-escape vector. Reject it BEFORE eval rather than trust
        # the empty-__builtins__ dict (which does NOT sandbox eval on its own).
        # Evaluate with the no-eval safe parser. Anything outside the boolean/
        # numeric grammar (identifiers, attributes, calls) is rejected -> False.
        try:
            return safe_eval_bool(cond)
        except SafeEvalError:
            logger.warning("yara_engine: rejected non-whitelisted condition in rule %r",
                           getattr(rule, "name", "?"))
            return False
        except Exception:
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ganglion-OOB YARA-like Rule Engine")
