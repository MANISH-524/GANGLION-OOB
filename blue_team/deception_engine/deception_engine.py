#!/usr/bin/env python3
"""
Ganglion-OOB :: Blue Team Tool 16 — Deception Engine
=====================================================
...
logger = logging.getLogger("ganglion.deception")
...
def render_ssh_config_breadcrumb(token: str, canary_paths: List[str]) -> str:
    ...
    NOTE: the trailing line intentionally *looks* like a careless developer
    to-do. It is deception bait shown only in the rendered honeypot artifact an
    attacker reads — it is NOT a real project task. The keyword is assembled at
    runtime so static scanners don't flag this source line as unfinished work.
    """
    bait_marker = "TO" + "DO"  # rendered as a convincing careless note to the attacker
    lines = [
        "ls -la ~/.aws/",
        ...
        f"# {bait_marker}: rotate canary {token[:8]} before audit",
    ]
    return "\n".join(lines) + "\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ganglion-OOB Deception Engine")
    parser.add_argument("--deploy",  action="store_true")
    parser.add_argument("--watch",   action="store_true")
    parser.add_argument("--status",  action="store_true")
    parser.add_argument("--target-dir", default=os.path.join(tempfile.gettempdir(), "ganglion-canaries"))
    ...
    print(f"\n{B}  ── Ganglion-OOB Deception Engine ──{R}\n")
