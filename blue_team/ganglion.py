#!/usr/bin/env python3
"""
Ganglion-OOB :: Unified Blue Team CLI
=======================================
Single entry point for the entire Ganglion-OOB Blue Team Detection Suite.
...
built-in workflows that chain multiple tools together automatically.
  ...
  ganglion tools                          List all available tools
  ganglion run <tool> [tool-args...]      Run a specific tool directly
  ganglion hunt                           Run threat_hunter with all playbooks
  ganglion audit                          Run config_auditor + ioc_hunter + file_integrity
  ganglion sweep <subnet>                 Run network_mapper + vuln_scanner on a subnet
  ganglion dashboard                      Launch the Master SOC Dashboard
  ganglion correlate <findings-dir>       Run alert_correlator across all findings
  ganglion report <findings-dir>          Generate executive + technical reports
  ganglion full-scan                      Run the complete detection pipeline end-to-end

All tool output (when run via 'ganglion run' with --json) is automatically
written into ./findings/<tool>_<timestamp>.json so the SOC dashboard and
alert_correlator pick it up immediately.

Usage:
    python3 ganglion.py tools
    python3 ganglion.py run threat_intel --query 8.8.8.8
    python3 ganglion.py hunt
    python3 ganglion.py audit
    python3 ganglion.py sweep 192.168.1.0/24
    python3 ganglion.py dashboard --port 8080
    python3 ganglion.py full-scan --target-dir /var/www
"""
...
logger = logging.getLogger("ganglion.cli")
...
TOOLS: Dict[str, dict] = {
    ...
    "soc_dashboard":               {"path": "soc_dashboard/soc_dashboard.py",
                                    ...
                                    "category": "Dashboard"},
    "waf_engine":                  {"path": "waf_engine/waf_engine.py",
                                    "desc": "Web app firewall — SQLi/XSS/RCE/SSRF/JNDI request inspection",
                                    "category": "Perimeter"},
    "ids_engine":                  {"path": "ids_engine/ids_engine.py",
                                    "desc": "Network IDS/IPS — signature + anomaly (scan/beacon/brute)",
                                    "category": "Perimeter"},
    "firewall":                    {"path": "firewall/firewall.py",
                                    "desc": "Config-driven firewall policy → OS containment enforcement",
                                    "category": "Perimeter"},
    "killchain_reconstructor":     {"path": "killchain_reconstructor/killchain_reconstructor.py",
                                    "desc": "Reconstruct ATT&CK kill-chain from detections; chain-completeness score",
                                    "category": "Correlation"},
    "blast_radius":                {"path": "blast_radius/blast_radius.py",
                                    "desc": "Reachability/blast-radius mapper; feeds asset-criticality to decisions",
                                    "category": "Correlation"},
}
...
def run_tool(name: str, extra_args: List[str], capture: bool = False) -> subprocess.CompletedProcess:
    if name not in TOOLS:
        ...
        print(f"  Run 'ganglion tools' to see available tools.")
        sys.exit(1)
...
def cmd_tools(args):
    ...
    print(f"\n{B}  GANGLION-OOB BLUE TEAM SUITE — {len(TOOLS)} TOOLS{R}\n")
    ...
    print(f"  Usage: ganglion run <tool> [args...]")
    print(f"  Example: ganglion run vuln_scanner --target 192.168.1.1\n")
...
def cmd_hunt(args):
    C = "\033[96m"; R = "\033[0m"; B = "\033[1m"
    print(f"\n{B}  ── Ganglion Hunt: Running all threat hunt playbooks ──{R}\n")
...
def cmd_audit(args):
    C = "\033[96m"; R = "\033[0m"; B = "\033[1m"
    print(f"\n{B}  ── Ganglion Audit: config_auditor + ioc_hunter + file_integrity ──{R}\n")
    ...
    print(f"\n  {C}Audit complete. Run 'ganglion correlate findings/' to unify results.{R}\n")
...
def cmd_sweep(args):
    C = "\033[96m"; R = "\033[0m"; B = "\033[1m"
    subnet = args.subnet
    print(f"\n{B}  ── Ganglion Sweep: network_mapper + vuln_scanner on {subnet} ──{R}\n")
    ...
    print(f"\n  {C}Sweep complete. Run 'ganglion correlate findings/' to unify results.{R}\n")
...
def cmd_report(args):
    C = "\033[96m"; R = "\033[0m"; B = "\033[1m"
    print(f"\n{B}  ── Ganglion Report: generating executive + technical reports ──{R}\n")
...
def cmd_full_scan(args):
    ...
    print(f"\n{B}  ════════════════════════════════════════════════")
    print(f"   GANGLION-OOB FULL DETECTION PIPELINE")
    print(f"  ════════════════════════════════════════════════{R}\n")
    ...
    print(f"  Next: ganglion report {FINDINGS_DIR}")
    print(f"  Next: ganglion dashboard\n")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ganglion",
        description="Ganglion-OOB Unified Blue Team CLI — 21 tools, one interface",
    )
