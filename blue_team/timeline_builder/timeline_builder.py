#!/usr/bin/env python3
"""
Ganglion-OOB :: Blue Team Tool 7 — Forensic Timeline Builder
=============================================================
...
Ingestion sources:
  ...
  - Ganglion-OOB alert JSON (from control_center.py)
...
Usage:
    ...
    python3 timeline_builder.py --ganglion-alerts alerts.json --yara-matches matches.json
...
logger = logging.getLogger("ganglion.timeline")
...
class JSONAlertSource:
    """Ingest Ganglion-OOB alerts, YARA matches, or IOC findings as JSON."""
...
class Timeline:
    ...
    def to_html(self, path: str, events: List[TimelineEvent] = None):
        ...
        <meta charset="UTF-8"><title>Ganglion-OOB Forensic Timeline</title>
        <style>
        ...
        <h1>⬡ GANGLION-OOB · FORENSIC TIMELINE</h1>
    ...
    def to_markdown(self, path: str, events: List[TimelineEvent] = None):
        ev_list = events or self.events
        lines = ["# Ganglion-OOB Forensic Timeline\n",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ganglion-OOB Timeline Builder")
    ...
    parser.add_argument("--ganglion-alerts", help="Ganglion alert JSON file")
    ...
    print(f"\n{B}  ── Ganglion-OOB Timeline Builder ──{R}\n")
    ...
    if args.ganglion_alerts:
        tl.add_source(JSONAlertSource(args.ganglion_alerts, "alert"))
