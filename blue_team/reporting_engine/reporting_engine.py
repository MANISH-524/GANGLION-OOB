#!/usr/bin/env python3
"""
Ganglion-OOB :: Blue Team Tool 18 — Reporting Engine
=====================================================
...
logger = logging.getLogger("ganglion.reporting")
...
<title>Ganglion-OOB — Executive Security Report</title>{HTML_STYLE}</head>
<body><div class="page">
<h1>⬡ GANGLION-OOB SECURITY REPORT</h1>
...
<footer>Ganglion-OOB · Out-of-Band Cyber Resilience Framework ·
...
def build_technical_html(findings: List[dict], risk: RiskSummary,
    ...
    return f"""<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><title>Ganglion-OOB — Technical Security Report</title>
{HTML_STYLE}</head><body><div class="page">
<h1>⬡ GANGLION-OOB TECHNICAL REPORT</h1>
...
<footer>Ganglion-OOB Technical Report · {generated_at}</footer>
</div></body></html>"""
...
def build_compliance_html(findings: List[dict], risk: RiskSummary,
    ...
    return f"""<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><title>Ganglion-OOB — Compliance Report ({framework.upper()})</title>
{HTML_STYLE}</head><body><div class="page">
<h1>⬡ GANGLION-OOB COMPLIANCE REPORT</h1>
...
<footer>Ganglion-OOB Compliance Report · {generated_at}</footer>
</div></body></html>"""
...
def build_markdown_report(findings: List[dict], risk: RiskSummary,
    ...
    lines = [
        f"# Ganglion-OOB Security Report — {report_type.title()}",
        f"",
    ...
    lines += [
        ...
        "",
        "---",
        f"*Ganglion-OOB — Out-of-Band Cyber Resilience Framework*",
    ]
    return "\n".join(lines)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ganglion-OOB Reporting Engine")
    ...
    print(f"\n{B}  ── Ganglion-OOB Reporting Engine ──{R}\n")
    ...
    output = args.output or f"ganglion_{args.type}_report{ext}"
