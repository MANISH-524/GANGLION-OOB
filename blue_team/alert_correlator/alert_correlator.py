#!/usr/bin/env python3
"""
Ganglion-OOB :: Blue Team Tool 13 — Alert Correlator & Unification Engine
==========================================================================
...
tools. Ganglion solves this with:
  ...
  4. KILL-CHAIN RECONSTRUCTION — MITRE ATT&CK tactics have a natural
     ...
     Ganglion flags these as RECONSTRUCTED CAMPAIGNS with a campaign score
...
Usage:
    python3 alert_correlator.py --ingest-dir /var/ganglion/findings/
...
logger = logging.getLogger("ganglion.correlator")
...
class ToolAdapters:
    ...
    @staticmethod
    def ganglion_alerts(data) -> List[UnifiedFinding]:
        ...
        for vm in vms:
            ...
            for ev in vm.get("recent_events", []):
                if ev.get("event_type") == "heartbeat":
                    continue
                out.append(UnifiedFinding(
                    tool="ganglion_sentry", finding_type=ev.get("event_type","alert"),
...
ADAPTER_SIGNATURES: List[Tuple[str, callable]] = [
    ("ganglion_alerts",  lambda d: isinstance(d, dict) and "vms" in d),
...
class CorrelatorHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/findings":
            ...
            self._json({"status":"ok","service":"ganglion-correlator"})
        else:
...
def serve(engine: CorrelationEngine, host="127.0.0.1", port=7100):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ganglion-OOB Alert Correlator")
    ...
    print(f"\n{B}  ── Ganglion-OOB Alert Correlator ──{R}\n")
