#!/usr/bin/env python3
"""
Ganglion-OOB :: Master SOC Dashboard
=====================================
...
Usage:
    python3 soc_dashboard.py --findings-dir /var/ganglion/findings --port 8080
...
logger = logging.getLogger("ganglion.soc_dashboard")
...
TOOL_REGISTRY = {
    ...
    "ganglion_sentry":            {"icon": "👁️",  "category": "OOB Agent",  "color": "#e2e8f0"},
}
...
<title>GANGLION-OOB :: Master SOC Dashboard</title>
...
<nav class="topbar">
  <div class="logo">
    ...
    <div>
      <div class="logo-text">GANGLION-OOB</div>
...
const TOOL_META = {
  ...
  ganglion_sentry:{icon:"👁️",color:"#e2e8f0"},
};
...
def create_app(state: DashboardState) -> Flask:
    ...
    @app.route("/healthz")
    def healthz():
        return jsonify({"status": "ok", "service": "ganglion-soc-dashboard"})
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ganglion-OOB Master SOC Dashboard")
    ...
    print(f"\n{B}  ── Ganglion-OOB Master SOC Dashboard ──{R}")
